...
namespace Clr2Jvm.Interop
{
    ...
    public class JavaInstall
    {
        ...
        readonly Native.Jni jni;
        readonly object syncRoot = new();
        readonly JavaOptions defaultOptions;
        readonly Lazy<JavaRuntime> runtime;
        readonly ConcurrentDictionary<Native.JniInvokeInterfacePointer, JavaRuntime> runtimes = new();
        ...
        public JavaInstall(JavaOptions defaultOptions = null, string jvmLibName = "jvm")
        {
            ...
            this.jni = new Native.Jni(Path.HasExtension(jvmLibName) ? NativeLibrary.Load(jvmLibName) : NativeLibrary.Load(jvmLibName, typeof(JavaInstall).Assembly, DllImportSearchPath.SafeDirectories));
        }
        ...
        unsafe void LoadInstances()
        {
            lock (syncRoot)
            {
                var vms = (Span<Native.JniInvokeInterfacePointer>)stackalloc Native.JniInvokeInterfacePointer[16];
                jni.GetCreatedJavaVMs(vms, out var nVMs);

                // initialize new JRE instances, and track those remaining
                var del = new HashSet<Native.JniInvokeInterfacePointer>(runtimes.Keys);
                for (var i = 0; i < nVMs; i++)
                    del.Remove(runtimes.AddOrUpdate(vms[i], _ => new JavaRuntime(this, null, new Native.JavaVM(_)), (_, __) => __).VM.Handle);
        ...
        unsafe JavaRuntime CreateJavaRuntime(JavaOptions options)
        {
            ...
            var args = new Native.JavaVMInitArgs();
            ...
            var enc = Native.JavaUtf8Encoding.Default;
            var osz = optsList.Count + 1;
            var opt = osz > 128 ? new Native.JavaVMOption[osz] : stackalloc Native.JavaVMOption[osz];
            ...
            try
            {
                ...
                try
                {
                    ...
                    return new JavaRuntime(this, options, new Native.JavaVM(vm_ptr));
                }
                catch (Native.JniException e)
                {
